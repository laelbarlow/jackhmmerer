...
# Script for setting up jackhmmerer.
...
PYENVDIR=~/env_jackhmmerer_workflow_setup_py
...
if [ "$(uname)" == "Darwin" ]; then
    ...
    # SnakeMake profile.
    if [ "$snakemake_profile" == "slurm_jackhmmerer" ]; then
        ...
        if [ ! -d ~/.config/snakemake/slurm_jackhmmerer ]; then
            rm -rf ~/.config/snakemake/slurm_jackhmmerer
        fi

        mkdir -p ~/.config/snakemake
        cd ~/.config/snakemake
        cookiecutter https://github.com/Snakemake-Profiles/slurm.git \
            profile_name="slurm_jackhmmerer" \
        ...
        ~/.config/snakemake/slurm_jackhmmerer/cluster_config.yaml
        ...
        # Copy over the slurm-jobscript.sh file in the profile directory (this
        ...
        ~/.config/snakemake/slurm_jackhmmerer/slurm-jobscript.sh

    elif [ "$snakemake_profile" == "sge_jackhmmerer" ]; then
        ...
        # Removing existing directory, if present.
        if [ ! -d ~/.config/snakemake/sge_jackhmmerer ]; then
            rm -rf ~/.config/snakemake/sge_jackhmmerer
        fi
        ...
        cookiecutter https://github.com/Snakemake-Profiles/sge.git \
            profile_name="sge_jackhmmerer"
        # Make Python scripts in profile executable.
        chmod a+x ~/.config/snakemake/sge_jackhmmerer/*.py
        # Change back to previous directory.
        cd -
        # Now you can run snakemake as: snakemake --profile sge_jackhmmerer ...


    elif [ "$snakemake_profile" == "pbs-torque_jackhmmerer" ]; then
        ...
        # Removing existing directory, if present.
        if [ ! -d ~/.config/snakemake/pbs-torque_jackhmmerer ]; then
            rm -rf ~/.config/snakemake/pbs_torque_jackhmmerer
        fi
        ...
        cookiecutter https://github.com/Snakemake-Profiles/pbs-torque.git \
            profile_name="pbs-torque_jackhmmerer"
        # Make Python scripts in profile executable.
        chmod a+x ~/.config/snakemake/pbs-torque_jackhmmerer/*.py
        # Change back to previous directory.
        cd -
        # Now you can run snakemake as: snakemake --profile
        # pbs-torque_jackhmmerer ...
