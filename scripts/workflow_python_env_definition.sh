...
DIR=~/env_jackhmmerer_snakemake_workflow
...
if test "$(command -v conda)"; then
  ...
  if ! test "$(conda info --envs | grep "conda_env_jackhmmerer_snakemake_workflow")"; then
    #conda create --name conda_env_jackhmmerer_snakemake_workflow python=3.7.4
    #source activate conda_env_jackhmmerer_snakemake_workflow
